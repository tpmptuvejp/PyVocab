...
def save_concepts(concepts, filename):
    try:
        ...
        update_chat_history(f"Error saving concepts: {e}")
...
def add_concept(concepts):
    def save_new_concept():
        nonlocal concept_entry, explanation_entry, example_entry, category_entry
        ...
        update_chat_history(f"Concept '{concept}' in category '{category}' has been added successfully!")
    ...
    category_label = tk.Label(dialog_window, text="Category:")
    category_label.grid(row=0, column=0, padx=10, pady=5, sticky="w")
    category_entry = tk.Entry(dialog_window)
    category_entry.grid(row=0, column=1, padx=10, pady=5, sticky="ew")

    concept_label = tk.Label(dialog_window, text="Concept Name:")
    concept_label.grid(row=1, column=0, padx=10, pady=5, sticky="w")
    concept_entry = tk.Entry(dialog_window)
    concept_entry.grid(row=1, column=1, padx=10, pady=5, sticky="ew")

    explanation_label = tk.Label(dialog_window, text="Explanation:")
    explanation_label.grid(row=2, column=0, padx=10, pady=5, sticky="w")
    explanation_entry = tk.Entry(dialog_window)
    explanation_entry.grid(row=2, column=1, padx=10, pady=5, sticky="ew")

    example_label = tk.Label(dialog_window, text="Example:")
    example_label.grid(row=3, column=0, padx=10, pady=5, sticky="w")
    example_entry = tk.Entry(dialog_window)
    example_entry.grid(row=3, column=1, padx=10, pady=5, sticky="ew")

    add_button = tk.Button(dialog_window, text="Add Concept", command=authenticate_admin)
    add_button.grid(row=4, columnspan=2, padx=10, pady=10, sticky="ew")
...
update_chat_history("You can ask me some of the python concepts that you don't understand, I will try to help")
update_chat_history("You can also add new concepts by clicking the button below this text window(admin access required).\n")
